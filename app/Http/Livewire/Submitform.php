...
class Submitform extends Component
{
    ...
    public $fees = [
        "Science" => 500, "Business Studies" => 400, "Humanities" => 300
    ];
    protected $rules = [
...
//        dd($this->ssc_info);
    ...
    {
        $registrationData = RegistrationForm::with('transaction')->where('user_id', Auth::id())->first();
        if ($registrationData) {
            return view('students.dashboard')->with(['registrationData'=>$registrationData]);
        }
    ...
    {
//        dd($this->fees[$this->ssc_info->hsc_group]);
        try {
            $this->form["photo"] = $this->uploadedPhotoUrl;
            $this->form["user_id"] = Auth::id();
            $this->form["transaction_number"] = date("Y") . $this->ssc_info['roll'];
            $this->form["transaction_amount"] = $this->fees[$this->ssc_info->hsc_group];
//            dd($this->form);exit;
            // Validate form data
//            $this->ssc_info->save();
...
//            $this->form->save(); // Automatically updates created_at and updated_at fields
            ...
            $this->ssc_info->save();
            Transactions::create(['transaction_number' => $this->form["transaction_number"],
                'transaction_amount' => $this->form["transaction_amount"], 'user_id' => Auth::id()]);
            // Success message
